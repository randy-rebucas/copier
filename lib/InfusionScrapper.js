...
class InfusionScrapper {
     ...
     * @param {int} limit 
     */
    constructor(connection, token, module, limit = 100) {
 ...
 */
    ...
    async getLog() {
        return new Promise((resolve, reject) => {
            ...
            return this.connection.query(maxQuery, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results)
    ...
    async createLog(lastId) {
        return new Promise((resolve, reject) => {
            var log = { offset: (this.page - 1) * this.limit, last_id: lastId, type: this.module};
            return this.connection.query(
    ...
    async checkLastLog() {
        return new Promise((resolve, reject) => {
            return this.connection.query(
                `SELECT EXISTS(SELECT * from logs WHERE 'offset'=? AND type='${this.module}') AS isExistOffset`,
